...
/// Stable topological auto-sort (the LOOT core rules): masters tier first,
/// and within each tier every plugin loads after its masters, otherwise
/// keeping the current relative order. Cycles keep their current order.
#[must_use]
pub fn auto_sort(plugins: &[GamePlugin]) -> Vec<String> {
    let mut order = Vec::with_capacity(plugins.len());
    for tier in [true, false] {
        let members: Vec<&GamePlugin> = plugins.iter().filter(|p| p.is_master == tier).collect();
...
pub fn render_plugins_txt(plugins: &[GamePlugin]) -> String {
    let mut out = String::from("# Managed by ModManager\n");
...
mod tests {
    ...
    fn renders_plugins_txt_with_activation_stars() {
        ...
        assert_eq!(text, "# Managed by ModManager\n*a.esp\nb.esp\n");
    }
}
