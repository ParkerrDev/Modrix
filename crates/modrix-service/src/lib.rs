...
//! shared by `modrix serve` (headless) and the GUI (embedded): it accepts
...
pub fn fomod_pass(engine: &Engine, staged: &modrix_core::Mod) -> Result<bool> {
...
pub enum Binding {
    ...
    Primary {
        ...
        serving: tokio::task::JoinHandle<modrix_ipc::Result<()>>,
    },
    /// Another Modrix instance already holds the port; it is the one
...
impl Service {
    ...
    pub fn bind(&self, lockfile: &Path, port: u16) -> Result<Binding> {
        match acquire(lockfile, port).context("acquiring the instance lock")? {
            Role::Secondary(_) | Role::PortTaken => Ok(Binding::AlreadyRunning),
...
fn state_str(status: &DownloadStatus) -> &'static str {
    use modrix_download::DownloadState::{Active, Complete, Failed, Paused, Queued};
    match status.state {
        Queued => "queued",
        Active => "active",
        Paused => "paused",
...
mod tests {
    ...
    fn install_file_reports_staging_failures() {
        ...
        let paths = modrix_core::Paths::rooted_at(dir.path());
        let engine = Engine::open(&paths).expect("open engine");
        let def = modrix_core::GameDef::from_toml_str(
            "api_version = 1\nid = \"g\"\nname = \"G\"\nmod_root = \"Data\"\n",
