...
//! later, in `modrix-plugin`; this stays a small, dependency-light parser so the
...
mod tests {
    ...
    #[test]
    fn shipped_subnautica_definition_is_valid() {
        // The built-in Subnautica def is a Unity/BepInEx game: mods deploy into
        // the plugin container (a nested mod root, which also preserves the
        // `BepInEx` casing) and there is no load-order strategy - BepInEx orders
        // plugins by their declared dependencies.
        ...
        assert_eq!(def.nexus_domain.as_deref(), Some("subnautica"));
