...
impl Harness {
    ...
    fn deploy(
        ...
        let dirty = crate::deploy::fsops::dirty_targets(&self.target_root, &current);
        let p = plan(
            self.game,
            crate::deploy::plan::Roots {
                target: self.target_root.clone(),
                backup: self.paths.backup_root(),
            },
            ordered,
            &crate::deploy::plan::CurrentState {
                rows: &current,
                dirty: &dirty,
            },
            &crate::deploy::plan::Overrides::default(),
        );
...
// ---------------------------------------------------------------------------
// A store-file edit after deploy reaches the game on the next deploy, even
// though the edit's write-rename broke the hardlink (the skee64.ini bug: the
// planner used to keep the row because the source *path* was unchanged).
// ---------------------------------------------------------------------------
#[test]
fn edited_store_file_is_redeployed_not_kept() {
    let mut h = Harness::new();
    let (id, resolved) = h.add_mod(&[("skse/plugins/skee64.ini", b"bImmediateFace=0")]);
    let ordered = vec![(id, resolved.clone())];
    h.deploy(&ordered, &Faults::none()).unwrap();

    // Edit the staged source the way editors do: write a new file and rename
    // it over the old one, which breaks the deployed hardlink.
    let source = &resolved.first().unwrap().source;
    let tmp = source.with_extension("new");
    fs::write(&tmp, b"bImmediateFace=1").unwrap();
    fs::rename(&tmp, source).unwrap();

    let report = h.deploy(&ordered, &Faults::none()).unwrap();
    assert_eq!(report.added(), 1, "the edited file must be re-placed");
    assert_eq!(
        h.tree().get("skse/plugins/skee64.ini").unwrap(),
        b"bImmediateFace=1",
        "the store edit must reach the game"
    );

    // And the refreshed deployment is stable: nothing to do next time.
