...
/// Per-file winner pins: lowercased target path → the mod that must provide
/// it (a Vortex "individual file" rule). An override naming a mod that does
/// not contest the target is inert.
pub(crate) type Overrides = HashMap<String, ModId>;

/// Where a plan deploys to and keeps its displaced-original backups.
pub(crate) struct Roots {
    /// The game's deploy root.
    pub target: PathBuf,
...
/// The observed deployment the planner diffs against: the manifest rows plus
/// the caller's I/O verdict on which of them have a staged source that drifted
/// since deploy (see `fsops::source_out_of_sync`). Dirty targets are re-placed
/// instead of kept, so a store-file edit reaches the game.
pub(crate) struct CurrentState<'a> {
    /// The game's current manifest rows.
    pub rows: &'a [DeployedRow],
    /// Targets among `rows` whose source is out of sync with the deploy.
...
pub(crate) fn plan(
    ...
    let (desired, conflicts) = resolve_tree(ordered_mods, overrides);
    ...
    for (target, win) in &desired {
        match current_by_target.get(target.as_str()) {
            Some(row)
                if row.source == win.source
                    && row.mod_id == win.mod_id
                    && !dirty.contains(target.as_str()) =>
            {
                keep.push((*row).clone());
            }
...
/// The last mod in order wins unless a file override pins the target to a
/// specific contender; every losing mod is listed (invariant I5:
/// deterministic, and nothing is hidden).
fn resolve_tree(
    ordered_mods: &[(ModId, Vec<ResolvedFile>)],
    overrides: &Overrides,
) -> (BTreeMap<String, Winner>, Vec<Conflict>) {
    // Every contender per target, in mod order (a mod providing the same
    // target twice keeps only its last file).
    let mut contenders: BTreeMap<String, Vec<Winner>> = BTreeMap::new();
    for (mod_id, files) in ordered_mods {
        for file in files {
            let slot = contenders.entry(file.target_rel.clone()).or_default();
            let winner = Winner {
                mod_id: *mod_id,
                source: file.source.clone(),
            };
            match slot.last_mut() {
                Some(last) if last.mod_id == *mod_id => *last = winner,
                _ => slot.push(winner),
            }
        }
    }
    ...
    for (target, slot) in contenders {
        let pinned = overrides
            .get(&target.to_ascii_lowercase())
            .and_then(|m| slot.iter().position(|w| w.mod_id == *m));
        let win = pinned.unwrap_or(slot.len().saturating_sub(1));
        let Some(winner) = slot.get(win) else {
            continue;
        };
        if slot.len() > 1 {
        ...
        desired.insert(target, winner.clone());
    }
...
mod tests {
    ...
    fn roots() -> Roots {
        Roots {
            target: PathBuf::from("/game"),
            backup: PathBuf::from("/backups"),
        }
    }

    fn current(rows: &[DeployedRow]) -> CurrentState<'_> {
        static NO_DIRTY: std::sync::OnceLock<HashSet<String>> = std::sync::OnceLock::new();
        CurrentState {
            rows,
            dirty: NO_DIRTY.get_or_init(HashSet::new),
        }
    }
    ...
    #[test]
    fn a_file_override_pins_the_winner() {
        let mods = vec![
        ...
        assert_eq!(p.conflicts()[0].winner, m(1));
        assert_eq!(p.conflicts()[0].shadowed, vec![m(2)]);
    }

    #[test]
    fn an_override_naming_a_non_contender_is_inert() {
        let mods = vec![
            (m(1), vec![file("shared.esp", "/s/1/shared")]),
            (m(2), vec![file("shared.esp", "/s/2/shared")]),
        ];
        let overrides: Overrides = [("shared.esp".to_owned(), m(9))].into_iter().collect();
        let p = plan(game(), roots(), &mods, &current(&[]), &overrides);
        assert_eq!(p.conflicts()[0].winner, m(2));
    }
    ...
    #[test]
    fn a_dirty_target_is_replaced_not_kept() {
        let mods = vec![(m(1), vec![file("a", "/s/1/a")])];
        let rows = vec![deployed(1, "a", "/s/1/a")];
        let dirty: HashSet<String> = [String::from("a")].into();
        let p = plan(
            game(),
            roots(),
            &mods,
        ...
        assert_eq!(p.to_add(), 1);
        assert_eq!(p.unchanged(), 0);
        // The prior row is carried so backups survive the replace.
        assert!(p.adds.first().is_some_and(|a| a.prior.is_some()));
    }
