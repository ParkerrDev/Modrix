...
pub(super) fn body(app: &App) -> El<'_> {
    ...
    if let Some(conflicts) = conflicts(app) {
        page = page.push(conflicts);
    }
...
fn toolbar(app: &App) -> El<'_> {
    ...
    let mut bar = row![
        text(format!("{active} of {count} active"))
            .size(12)
            .color(theme::MUTED),
...
fn conflicts(app: &App) -> Option<El<'_>> {
    if app.conflicts.is_empty() {
        return None;
    }
    let mut list = column![].spacing(4);
    for c in app.conflicts.iter().take(10) {
        list = list.push(
            row![
    ...
    let extra = app.conflicts.len().saturating_sub(10);
    if extra > 0 {
        list = list.push(text(format!("+{extra} more")).size(11).color(theme::FAINT));
    }
    Some(
        container(column![text("FILE CONFLICTS").size(10).color(theme::FAINT), list].spacing(8))
            .padding([10, 14])
            .width(Length::Fill)
            .style(theme::inset)
            .into(),
