...
pub(super) fn body(app: &App) -> El<'_> {
    ...
    if app.downloads.is_empty() {
        ...
        for status in &app.downloads {
            rows = rows.push(download_row(status));
        }
        page = page.push(
...
fn download_row(status: &DownloadStatus) -> El<'_> {
    let name = status
        .file
        .file_name()
        .map_or_else(|| "download".to_owned(), |n| n.to_string_lossy().into_owned());
    let (color, label) = state_badge(status.state);
    let head = row![
