...
//! single-instance lockfile, connects to the running ModManager instance over
...
async fn forward(uri: &str, data_dir: Option<&Path>) -> Result<()> {
    ...
    let reply = secondary
        .send("/nxm", uri)
        .await
        .context("forwarding the link to ModManager")?;
    if reply.status == 200 {
        Ok(())
    } else {
        bail!(
            "ModManager rejected the link (HTTP {}): {}",
            reply.status,
            reply.body
        );
    }
...
fn usage() -> Result<()> {
    ...
    writeln!(
        out,
        "modman-protocol - forwards nxm:// links to ModManager\n\n\
         usage:\n  modman-protocol <nxm://…>   forward a download link\n  \
         modman-protocol --register     register as the OS nxm:// handler\n  \
         modman-protocol --unregister   remove the registration"
    )?;
    Ok(())
}
...
mod register {
    ...
    mod linux {
        ...
        const DESKTOP_FILE: &str = "modman-nxm.desktop";
