...
//! (or `modman-protocol` forwarding an `nxm://` link) finds the port taken,
...
/// secondary (or `modman-protocol`) can authenticate to the primary.
///
/// # Errors
///
...
/// how `modman-protocol` reaches a running primary to forward an `nxm://` link.
///
/// # Errors
///
...
mod tests {
    ...
    async fn second_acquire_on_same_port_is_secondary() {
        ...
        match acquire(&lock, port).unwrap() {
            ...
            Role::Primary(_) => panic!("second acquire must be secondary"),
        }
    }
}
